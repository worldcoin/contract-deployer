...
pub async fn deploy(context: &Context, config: &Config) -> eyre::Result<()> {
    ...
    let verifier_abi = ForgeInspectAbi::new(ContractSpec::name("VerifierLookupTable"))
        .with_cwd("./world-id-contracts")
        .run()
        .await?;

    let insert_lookup_table = ForgeCreate::new(ContractSpec::name("VerifierLookupTable"))
        .with_cwd("./world-id-contracts")
    ...
    let signer = context.typed_map.read().await.get::<RpcSigner>().0.clone();
    ...
    let insertion_verifier = context
        .typed_map
        .read()
        .await
        .get::<InsertionVerifier>()
        .clone();
